...
public class PeriodicallyReCachedValue<T> implements CachedChangingValue<T> {
    ...
    private final AtomicReference<Long> lastReCacheTimestamp = new AtomicReference<>();
    ...
    @Override
    public long getLastReCacheTimestamp() {
        return lastReCacheTimestamp.get();
    }
    ...
    public void forceNewValueReCaching(boolean ignoreDifferenceCheck) {
        synchronized (currentValueHolder) {
            ...
            lastReCacheTimestamp.set(System.currentTimeMillis());
        }
    }
}
